...
const HeroSection: React.FC = () => (
  <section id="hero" className="relative overflow-hidden bg-[#f4f5f7] snap-start">
    <div className="hero-grid-bg pointer-events-none opacity-40" aria-hidden="true" />
    <div className="pointer-events-none absolute left-[-12%] top-[-8%] h-72 w-72 rounded-full bg-cyan-500/12 blur-3xl" aria-hidden="true" />
    <div className="pointer-events-none absolute right-[-10%] bottom-[-10%] h-80 w-80 rounded-full bg-violet-500/12 blur-3xl" aria-hidden="true" />

    <div className="hero-inner">
      <div className="grid items-center gap-12 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <div className="space-y-6 max-w-3xl">
          <div className="inline-flex flex-wrap items-center gap-2 rounded-full bg-white/80 px-4 py-2 text-xs font-semibold uppercase tracking-[0.25em] text-slate-700 ring-1 ring-slate-200 backdrop-blur">
            <span>Cloud Security · Web VAPT · Network &amp; Infra</span>
          </div>

          <h1 className="hero-title space-y-3 leading-tight tracking-tight text-slate-900">
            <span className="hero-line text-balance text-3xl md:text-4xl lg:text-5xl">
              앞에서는 서비스, 뒤에서는 보안.
            </span>
            <span className="hero-line text-balance text-3xl md:text-4xl lg:text-5xl">
              <span className="brush-text">모르는 건 빠르게 만들면서 배우는</span> 서상원입니다.
            </span>
          </h1>
          ...
          <div className="brush-frame inline-flex items-center gap-2 px-4 py-3 text-sm font-semibold text-slate-800">
            Cloud Security · Web VAPT · Network &amp; Infra
          </div>

          <div className="flex flex-wrap gap-2 text-xs text-slate-700 md:text-sm">
            {metrics.map((m) => (
              <span key={m.label} className="inline-flex items-center gap-2 rounded-full bg-white/90 px-3 py-2 ring-1 ring-slate-200 shadow-sm">
        ...
        <aside className="relative flex justify-end">
          ...
          <div className="relative z-10 w-full max-w-[460px] rounded-[28px] bg-white/90 p-4 ring-1 ring-slate-200 shadow-[0_25px_90px_rgba(15,23,42,0.12)] backdrop-blur-lg">
            <div className="overflow-hidden rounded-3xl bg-slate-100 ring-1 ring-slate-200">
              <img src="/main-me.png" alt="서상원 프로필" className="h-full w-full object-contain" />
            </div>
            <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-slate-900/80 px-3 py-1 text-[0.7rem] font-semibold text-slate-100 ring-1 ring-slate-800/70">
